...
| Copy and art provenance | `npm run verify:content` | 73 text files, no prohibited dash characters or first-party SVG files |
...
| Browser suite | `npx playwright test` | 34 stories passed, 2 unsupported service-worker stories skipped in 1.4 minutes |
...
The Level 01 redesign adds `release/qa/level-01-3d/desktop-1440x900.png`, `desktop-solved-final.png`, `mobile-390x844.png`, and a 14.3-second `gameplay.webm`. System Chrome reported no page or console errors. The 1440 by 900 scene measured 144 frames per second on a 144 Hz display, 125 draw calls, and 47,182 triangles. The 390 by 844 scene measured 144 frames per second, 124 draw calls, and 47,014 triangles. These are warm local measurements, not a hardware-wide benchmark.
...
The Level 01 pass checked continuous playback, exact wait holds, timeline scrubbing, route changes, solution completion, keyboard playback, reduced-motion playback suppression, vehicle selection, desktop fit without scrolling, mobile horizontal fit, scene loading, and the Level 02 transition. Foreground buildings and trees were moved after the first capture so they no longer hide the robot or shared bridge.
