...
try {
  ...
  await page.getByText('An update is ready.', { exact: false }).waitFor({ timeout: 20_000 });
  await page.getByRole('button', { name: 'Update now' }).click();
  await page.getByRole('button', { name: 'Garden path', exact: true }).waitFor();
  if (await page.getByRole('button', { name: 'Garden path', exact: true }).getAttribute('aria-pressed') !== 'true') throw new Error('The save did not survive the update.');
  await browser.close();
